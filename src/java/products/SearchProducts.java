    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("searchType") != null) {
            if (request.getParameter("searchType").equals("name")) {
                ...
                try {
                    productsList = pro.searchByName(productName);
                } catch (SQLException ex) {
                    Logger.getLogger(SearchProducts.class.getName()).log(Level.SEVERE, null, ex);
                }
            } else {
                int productPrice = Integer.parseInt(request.getParameter("search"));
                try {
                    productsList = pro.searchByPrice(productPrice);
                } catch (SQLException ex) {
                    Logger.getLogger(SearchProducts.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            String productsType = new String("Find Products");
